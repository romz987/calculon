...
def ozprice_request(stuff, tab):
    ...
    for count, (
        ...
        # FBS vs FBO 
        if fbso[0] == "fbs":
            logistics = _logistics_oz_fbs(package)
            shipment = shipment[0]
        elif fbso[0] == "fbo":
            logistics = _logistics_oz_fbo(package)
            shipment = 0            
        ...
        result = _oz_calculate(tab, calcdata)
...
def _oz_calculate(tab, calcdata):
    """
...
def _price_calc_sole(calcdata, price):
    ...
    # ЛОГ
    logging.info(
        f'{Fore.GREEN}[ozcalc.py][price_calc_sole]{Style.RESET_ALL}'
    )
    # Последняя миля 
    lastmile = _lastmile(price) 
    # Расчет комиссий
